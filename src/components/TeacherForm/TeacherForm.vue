<template>
  <div :class="$style.root">
    ...
    <div :class="$style.item">
      <div :class="$style.label">
        <label for="teacher">Авторы</label>
      </div>
...
import { selectItemById, fetchItems, selectItems } from '@/store/teachers/selectors';
...
export default {
  ...
  setup(props, context) {
    ...
    const teacherList = computed(() => selectItems(store));
    const form = reactive({
      id: '',
      teacher: '',
      department: '',
    ...
    watchEffect(() => {
      const teacher = selectItemById( store,  props.id );
      Object.keys(teacher).forEach(key => {
        form[key] = teacher[key]
      })
    });
    ...
    return {
      teacherList,
      form,
      isValidForm: computed(() =>  !!(form.teacher && form.department)),
      onClick: () => {
